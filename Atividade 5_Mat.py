...
class Grafo:
    ...
    def busca_em_profundidade_ciclo(self, v, visitado, c_atual):
        visitado.add(v)
        self.vertices[v].prof_ent = True

        for a in self.estrutura.get(v, []):
            if a not in visitado:  
                ciclo = self.busca_em_profundidade_ciclo(a, visitado, c_atual + [(v, a)])
                if ciclo:
                    return ciclo
            elif c_atual and a != c_atual[0][0]: 
                return c_atual + [(v, a)]

        visitado.remove(v)
        self.vertices[v].prof_ent = False
        return None
...
def main():
    ...
    cam_uv, aresta_uv = passeio.encontrar_caminho_passeio(u, v, v_pass)
    if cam_uv:
        print("Caminho entre {} e {}:".format(u.r, v.r))
        for i, vertice in enumerate(cam_uv):
            print(vertice.r, end="")
            if i < len(cam_uv) - 1:
                aresta = aresta_uv[i]
                print(" ({} -> {})".format(aresta[0].r, aresta[1].r), end="")
                print(" -> ", end="")
        print()
    else:
